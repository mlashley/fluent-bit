                               ...
                               struct flb_es_config *ec)
{
    ...
    for (i = 0; i < map.via.map.size; i++) {
        ...
        if (v->type == MSGPACK_OBJECT_MAP) {
            ...
            es_pack_map_content(tmp_pck, *v, ctx,ec);
        }
        /*
         ...
         */
          ...
          es_pack_array_content(tmp_pck, *v, ctx,ec);
        }
        else {
...
struct flb_es_config *flb_elasticsearch_target(struct flb_elasticsearch *ctx,
                                               struct flb_upstream_node **node)
{
    ...
    if (ctx->ha_mode == FLB_TRUE) {
        ...
        ec = flb_upstream_node_get_data(f_node);
        *node = f_node;
    }
    else {
                                ...
                                void **out_data, size_t *out_size)
{
    ...
    struct flb_es_config *ec;
    struct flb_elasticsearch_flush *ef = flush_ctx;
    ...
    while (msgpack_unpack_next(&result, data, bytes, &off) == MSGPACK_UNPACK_SUCCESS) {
         ...
         */
        ret = es_pack_map_content(&tmp_pck, map, ctx,ec);
        if (ret == -1) {
            msgpack_unpacked_destroy(&result);
                      ...
                      void *data)
{
    ...
    if (tmp) {
        ret = elasticsearch_config_ha(tmp, ctx, config);
    }
    else {
        ret = elasticsearch_config_simple(ctx, ins, config);
    }
                        ...
                        struct flb_config *config)
{
    ...
    struct flb_upstream_node *node = NULL;
    struct flb_elasticsearch_flush *flush_ctx;
     ...
     * formatter function.
     */
    flush_ctx = flb_calloc(1, sizeof(struct flb_elasticsearch_flush));
    if (!flush_ctx) {
        flb_errno();
        FLB_OUTPUT_RETURN(FLB_RETRY);
    }
    ...
    if (ret != 0) {
        ...
        flb_plg_debug(ctx->ins, "HTTP Status=%i URI=%s Node=%s", c->resp.status, ec->uri,
                      (node) ? (node->name) : "n/a");
        if (c->resp.status != 200 && c->resp.status != 201) {
            if (c->resp.payload_size > 0) {
                flb_plg_error(ctx->ins, "HTTP status=%i URI=%s Node=%s, response:\n%s\n",
                              c->resp.status, ec->uri, (node) ? (node->name) : "n/a",
                              c->resp.payload);
            }
            else {
                flb_plg_error(ctx->ins, "HTTP status=%i URI=%s Node=%s",
                              c->resp.status, ec->uri, (node) ? (node->name) : "n/a");
            }
            goto retry;
        }
...
static int cb_es_exit(void *data, struct flb_config *config)
{
    ...
    mk_list_foreach_safe(head, tmp, &ctx->configs) {
        ec = mk_list_entry(head, struct flb_es_config, _head);
...
static struct flb_config_map config_map[] = {
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_es_config, index),
     NULL
    },
    {
     FLB_CONFIG_MAP_STR, "type", FLB_ES_DEFAULT_TYPE,
     0, FLB_TRUE, offsetof(struct flb_es_config, type),
     NULL
    },
    ...
    /* HTTP Authentication */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_es_config, http_user),
     NULL
    },
    {
     FLB_CONFIG_MAP_STR, "http_passwd", "",
     0, FLB_TRUE, offsetof(struct flb_es_config, http_passwd),
     NULL
    },
...
#ifdef FLB_HAVE_AWS
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_es_config, has_aws_auth),
     "Enable AWS Sigv4 Authentication"
    },
    {
     FLB_CONFIG_MAP_STR, "aws_region", "",
     0, FLB_TRUE, offsetof(struct flb_es_config, aws_region),
    ...
    /* Logstash compatibility */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_es_config, logstash_format),
     NULL
    },
    {
     FLB_CONFIG_MAP_STR, "logstash_prefix", FLB_ES_DEFAULT_PREFIX,
     0, FLB_TRUE, offsetof(struct flb_es_config, logstash_prefix),
     NULL
    },
    {
     FLB_CONFIG_MAP_STR, "logstash_prefix_key", NULL,
     0, FLB_TRUE, offsetof(struct flb_es_config, logstash_prefix_key),
     NULL
    },
    {
     FLB_CONFIG_MAP_STR, "logstash_dateformat", FLB_ES_DEFAULT_TIME_FMT,
     0, FLB_TRUE, offsetof(struct flb_es_config, logstash_dateformat),
     NULL
    },
    ...
    /* Custom Time and Tag keys */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_es_config, time_key),
     NULL
    },
    {
     FLB_CONFIG_MAP_STR, "time_key_format", FLB_ES_DEFAULT_TIME_KEYF,
     0, FLB_TRUE, offsetof(struct flb_es_config, time_key_format),
     NULL
    },
    {
     FLB_CONFIG_MAP_BOOL, "include_tag_key", "false",
     0, FLB_TRUE, offsetof(struct flb_es_config, include_tag_key),
     NULL
    },
    {
     FLB_CONFIG_MAP_STR, "tag_key", FLB_ES_DEFAULT_TAG_KEY,
     0, FLB_TRUE, offsetof(struct flb_es_config, tag_key),
     NULL
    },
    {
     FLB_CONFIG_MAP_SIZE, "buffer_size", FLB_ES_DEFAULT_HTTP_MAX,
     0, FLB_TRUE, offsetof(struct flb_es_config, buffer_size),
     NULL
    },
    ...
    },
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_es_config, generate_id),
     NULL
    },
    {
     FLB_CONFIG_MAP_BOOL, "replace_dots", "false",
     0, FLB_TRUE, offsetof(struct flb_es_config, replace_dots),
     NULL
    },

    {
     FLB_CONFIG_MAP_BOOL, "current_time_index", "false",
     0, FLB_TRUE, offsetof(struct flb_es_config, current_time_index),
     NULL
    },
    ...
    /* Trace */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_es_config, trace_output),
     NULL
    },
    {
     FLB_CONFIG_MAP_BOOL, "trace_error", "false",
     0, FLB_TRUE, offsetof(struct flb_es_config, trace_error),
     NULL
    },
    {
